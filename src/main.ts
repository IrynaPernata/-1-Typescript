...
/**
 * Інтерфейс для постів, які ми отримуємо з JSONPlaceholder
 */
interface Post {
  id: number;
  userId: number;
...
/**
 * Тип для категорій навичок
 */
type SkillCategory = 'frontend' | 'backend' | 'cloud' | 'emerging' | 'all';

/**
 * Інтерфейс для наших статичних даних про навички
 */
interface Skill {
  name: string;
  icon: string; 
  level: number;
  categories: SkillCategory[]; // Може належати до кількох категорій
}
 ...
 */
function initParticleEffect(): void {
