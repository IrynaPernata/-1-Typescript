...
function validateLesson(lesson) {
    const classroomConflict = schedule.find((l) => l.dayOfWeek === lesson.dayOfWeek &&
    ...
    const professorConflict = schedule.find((l) => l.dayOfWeek === lesson.dayOfWeek &&
        l.timeSlot === lesson.timeSlot &&
...
function addLesson(lesson) {
    ...
    if (conflict) {
        console.error(`Failed to add lesson ${lesson.id}. ${conflict.type} with existing lesson ${conflict.lessonDetails.id}.`);
        return false;
    }
...
function findAvailableClassrooms(timeSlot, dayOfWeek) {
    ...
    return allClassroomNumbers.filter((number) => !occupiedClassroomNumbers.includes(number));
}
/**
...
function getClassroomUtilization(classroomNumber) {
    ...
    const allTimeSlots = ["8:00-9:35", "9:45-10:20", "11:45-13:20", "13:30-15:05", "15:15-16:50"];
...
function reassignClassroom(lessonId, newClassroomNumber) {
    ...
    const conflict = schedule.find((l) => l.dayOfWeek === originalLesson.dayOfWeek &&
        l.timeSlot === originalLesson.timeSlot &&
        l.classroomNumber === newClassroomNumber);
    if (conflict) {
        console.warn(`Cannot reassign. New classroom ${newClassroomNumber} is occupied by lesson ${conflict.id}.`);
        return false;
    }
    // Оновлюємо аудиторію
...
console.log("Available classrooms on Monday at 8:00-9:35:", findAvailableClassrooms("8:00-9:35", "Monday"));
